...
| D47 | Copie de sauvegarde du plugin avant toute écriture : copie horodatée dans `DungeonMakerBackups/` à côté du plugin (extension `.bak`, ignorée par le jeu et le CK) ; l'écriture est refusée si le fichier a changé sur disque depuis son chargement | Décidé (24 sept. 2026) |
...
| D60 | Une **jonction est jugée sur ses deux profils** superposés (celui d'en face retourné et décalé), pas sur les types de connexion, dont la tolérance de regroupement (8 unités) laisse passer des coutures visibles de près : **exacte** (à 0,5 unité près ; mesuré : 0,0 sans couture, 1,0 avec), **incluse** (un profil contenu dans l'autre ; le surplus d'un côté, détail de plafond ou cadre autour d'un couloir plus étroit, est toléré), **couture** (seulement à la tolérance large, en jaune avec l'écart) ou **incompatible** (en rouge). Les types restent utilisés par l'assistant pour proposer des pièces | Décidé (23 sept. 2026) |
| D61 | Un **chevauchement voulu** se marque depuis l'éditeur (clic sur une cellule partagée magenta) : la paire de pièces est enregistrée dans les annotations par EditorID, avec sa position relative exacte (décalage du coin et rotation de la seconde pièce dans le repère de la première, indépendants de l'orientation de la paire). Partout où cette combinaison apparaît, elle n'est plus signalée, sa pose est acceptée et l'assistant peut la proposer. Liste et retrait dans Validation, onglet « Accepted overlaps » | Décidé (24 sept. 2026) |
| D62 | La V1 **crée aussi des plugins et des cellules** (étend l'hypothèse « édition de cellules existantes seulement ») : nouveau plugin vide (HEDR 1.71, premier id 0x800, masters = Skyrim.esm + les plugins qui fournissent des pièces du catalogue) dans un dossier de mod MO2 activé ou un nouveau dossier de mod (à activer dans MO2) ; nouvelle cellule intérieure minimale (EditorID, drapeau intérieur, éclairage par défaut) rangée dans son bloc / sous-bloc, écrite aussitôt avec copie de sauvegarde. L'éclairage réel se règle dans le CK | Décidé (24 sept. 2026) |
